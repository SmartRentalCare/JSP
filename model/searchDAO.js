...
function searchResultPost(parameters) {
    return new Promise((resolve, reject) => {
        let like = "%" + parameters;
        const queryData = `SELECT user, carNum, rentDay FROM renterInfo WHERE (carNum like ? OR user = ?)`;
...
function searchResultGet(parameters) {
    return new Promise((resolve, reject) => {
        ...
        const queryData = `SELECT user, carNum, rentDay FROM renterInfo WHERE (carNum like ? OR user = ?) LIMIT ?, ?`;
